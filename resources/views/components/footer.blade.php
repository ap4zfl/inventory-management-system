<footer class="py-5" style="border-top: 1px solid #eeecec;">
      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-md-6 col-sm-6">
          <div class="footer-menu">
            ...
            <h1>
              <a href="{{ url('/')}}">
                <span class="text-primary">I</span>M<span class="text-primary">S</span>
              </a>
             </h1>
        ...
        <div class="col-md-2 col-sm-6">
          <div class="footer-menu">
            ...
            <ul class="menu-list list-unstyled">
              @foreach(DB::table('categories')->orderBy('id', 'desc')->take(5)->get() as $category)
              <li class="menu-item">
                <a href="{{ url('/category/' . $category->cat_slug) }}" class="nav-link">{{ $category->cat_name }}</a>
              </li>
            @endforeach
            </ul>
          </div>
        ...
        <div class="col-md-2 col-sm-6">
          <div class="footer-menu">
            ...
            <ul class="menu-list list-unstyled">
              <li class="menu-item">
                <a href="{{ url('/shop')}}" class="nav-link">Trending</a>
              </li>
              <li class="menu-item">
                <a href="{{ url('/shop')}}" class="nav-link">Best Selling</a>
              </li>
              <li class="menu-item">
                <a href="{{ url('/shop')}}" class="nav-link">Most Popular</a>
              </li>
              <li class="menu-item">
                <a href="{{ url('/shop')}}" class="nav-link">Just arrived</a>
              </li>
            </ul>
          </div>
        </div>
...
<script>
   $(document).ready(function () {
                    ...
                    success: function (data) {
                        ...
                        if (data.length > 0) {
                            data.forEach(product => {
                                results.append(`
                                    <div class="search-item d-flex align-items-center mb-2">
                                        <a href="/product/${product.slug}" class="d-flex align-items-center text-decoration-none">
                                            <img src="{{ asset('${product.image}') }}" alt="${product.name}" style="width: 50px; height: 50px; margin-right: 10px;">
    ...
    // Extract price from the closest product item
    const priceText = $(this).closest('.product-item').find('.price').text().trim();
    const price = parseFloat(priceText.replace(/[^0-9.-]+/g, '')); // Remove non-numeric characters

    if (isNaN(price)) {
        console.error('Invalid price:', priceText); // Log invalid price for debugging
        return; // Stop if the price is invalid
    }

    // Prepare product data
    const product = {
        id: $(this).closest('.product-item').data('id'),
        name: $(this).closest('.product-item').find('h3').text().trim(),
        quantity: $(this).closest('.product-item').find('input[name="quantity"]').val(),
        price: price,
    ...
    $.ajax({
        ...
        error: function (xhr) {
            if (xhr.status === 403) {
                // Show error message
                $('.alert-danger').text(xhr.responseJSON.message).fadeIn().delay(3000).fadeOut();
            }
        }
    });
});
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.cart.forEach(item => {
                ...
                cartHtml += `
                    <li class="list-group-item d-flex justify-content-between lh-sm">
                        ...
                        <span class="text-muted">£${(price * quantity).toFixed(2)}</span>
                    </li>
                `;
            ...
            $('#cart-container .list-group').html(cartHtml);
            $('.cart-total').text(`£${cartTotal.toFixed(2)}`);
            $('.totalbalance').text(response.cart.length);
        }
    });
}
